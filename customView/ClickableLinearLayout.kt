...
class ClickableLinearLayout: LinearLayout {
    ...
    fun initView(attrs: AttributeSet?) {
        ...
        attrs?.let {
            val typedArray = context.obtainStyledAttributes(it, R.styleable.ClickableLinearLayout)
            rippleColor = typedArray.getColor(R.styleable.ClickableLinearLayout_rippleColor, Color.MAGENTA)
            rippleOpacity = typedArray.getInteger(R.styleable.ClickableLinearLayout_rippleOpacity, 100).takeIf { it in 0..100 } ?: 100
            cornerRadius = typedArray.getDimension(R.styleable.ClickableLinearLayout_cornerRadius, 0f)
            topLeftCornerRadius = typedArray.getDimension(R.styleable.ClickableLinearLayout_topLeftCornerRadius, 0f)
            topRightCornerRadius = typedArray.getDimension(R.styleable.ClickableLinearLayout_topRightCornerRadius, 0f)
            bottomLeftCornerRadius = typedArray.getDimension(R.styleable.ClickableLinearLayout_bottomLeftCornerRadius, 0f)
            bottomRightCornerRadius = typedArray.getDimension(R.styleable.ClickableLinearLayout_bottomRightCornerRadius, 0f)
            ripplePosition =
                if (typedArray.getInteger(R.styleable.ClickableLinearLayout_ripplePosition, 0) == 0) {
